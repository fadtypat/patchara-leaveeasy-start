...
// สัปดาห์ที่ 6: อ่านจริงจาก Firestore (ผ่าน window.getCollection ใน js/firebase-init.js)
// ─────────────────────────────────────────────────────────────
...
(async function () {
  ...
  try {
    ใบลาจากฐานข้อมูล = await window.getCollection("leaveRequests");
  } catch (err) {
    console.error(err);
    showConfigWarning("อ่านรายการใบลาจาก Firestore ไม่สำเร็จ");
    return;
  }

  // ใบลาจากฐานข้อมูลจริง บวกกับใบที่เพิ่งยื่นในหน้าถัดไป
  // (สัปดาห์นี้ยังไม่บันทึกใบใหม่ลงฐานข้อมูลจริง ใบที่ยื่นใหม่จึงหายเมื่อปิดเบราว์เซอร์)
